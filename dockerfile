...
RUN git clone https://github.com/patricoferris/riscv-ocaml -b 4.07+cross+custom riscv-ocaml-src
...
RUN cat ./configure
RUN ./configure --enable-ocamldoc=no --enable-debugger=no -prefix /riscv-ocaml && \
    make -j8 world.opt && make install
ENV PATH="/riscv-ocaml/bin:${PATH}"
RUN make clean && \
		./configure --target riscv64-unknown-linux-gnu -prefix /riscv-ocaml \
		--enable-ocamldoc=no --enable-debugger=no --with-target-bindir=/riscv-ocaml/bin && \
